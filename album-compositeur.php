...
<div class="container">
    <h2>LES ALBUMS CONCERNANT</h2>
    <p>Les albums concernant votre compositeur</p>
<?php
  ...
  $pdo = new PDO($pdodsn, $user, $password);
   $query = " select DISTINCT Album.Titre_Album, Album.Année_Album, Album.Code_Album 
              FROM Album INNER JOIN (Disque INNER JOIN (((Composition INNER JOIN ((Oeuvre INNER JOIN (Musicien INNER JOIN Composer ON Musicien.Code_Musicien = Composer.Code_Musicien) ON Oeuvre.Code_Oeuvre = Composer.Code_Oeuvre) INNER JOIN Composition_Oeuvre ON Oeuvre.Code_Oeuvre = Composition_Oeuvre.Code_Oeuvre) ON Composition.Code_Composition = Composition_Oeuvre.Code_Composition) INNER JOIN Enregistrement ON Composition.Code_Composition = Enregistrement.Code_Composition) INNER JOIN Composition_Disque ON Enregistrement.[Code_Morceau] = Composition_Disque.[Code_Morceau]) ON Disque.Code_Disque = Composition_Disque.Code_Disque) ON Album.Code_Album = Disque.Code_Album
